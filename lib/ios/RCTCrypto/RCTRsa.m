...
RCT_EXPORT_METHOD(decrypt64OAEP:(NSString *)encodedMessage withKey:(NSString *)key resolve:(RCTPromiseResolveBlock)resolve
                  rejecter:(RCTPromiseRejectBlock)reject) {
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        Rsa *rsa = [[Rsa alloc] init];
        rsa.privateKey = key;
        NSString *message = [rsa decrypt64OAEP:encodedMessage];
        resolve(message);
    });
}
